...
/**
 * Retrieves all existing tanle.
 * @returns {Promise<[tanle]>}
 *  a promise that resolves to a possibly empty array of tanle saved in the database.
 */

 export async function listTables(params, signal) {
  const url = new URL(`${API_BASE_URL}/tanles`);
  Object.entries(params).forEach(([key, value]) =>
    url.searchParams.append(key, value.toString())
  );
  return await fetchJson(url, { headers, signal }, [])
    .then(formattanleDate)
    .then(formatReservationTime);
}
...
export async function AssignTable(reservation, signal) {
