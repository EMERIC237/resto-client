 ...
 */
 export async function updateR(updatedR, signal) {
  const url = `${API_BASE_URL}/Rs/${updatedR.id}?_embed=cards`;
  const options = {
    method: "PUT",
    headers,
    body: JSON.stringify(stripCards(updatedR)),
    signal,
  };
  return await fetchJson(url, options, updatedR);
}
