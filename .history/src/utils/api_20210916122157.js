...
/**
 * Updates an existing reser
 * @param updatedReservat
 *  the reser to save, which must have an `id` property.
 * @param signal
 *  optional AbortController.signal
 * @returns {Promise<Error|*>}
 *  a promise that resolves to the updated reser.
 */
 export async function updateReservat(updatedReservat, signal) {
  const url = `${API_BASE_URL}/Reservats/${updatedReservat.id}?_embed=cards`;
  const options = {
    method: "PUT",
    headers,
    body: JSON.stringify(stripCards(updatedReservat)),
    signal,
  };
  return await fetchJson(url, options, updatedReservat);
}
