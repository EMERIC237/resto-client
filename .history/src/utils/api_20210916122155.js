 ...
 */
 export async function updateReser(updatedReser, signal) {
  const url = `${API_BASE_URL}/Resers/${updatedReser.id}?_embed=cards`;
  const options = {
    method: "PUT",
    headers,
    body: JSON.stringify(stripCards(updatedReser)),
    signal,
  };
  return await fetchJson(url, options, updatedReser);
}
